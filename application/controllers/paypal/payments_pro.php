...
class Payments_pro extends CI_Controller {
    ...
    function __construct() {
        ...
        // Load PayPal library
        $this->config->load('paypal-sample');
    ...
    function Do_capture() {
        $DCFields = array(
            'authorizationid' => '5JL11387UK260600F', // Required. The authorization identification number of the payment you want to capture. This is the transaction ID returned from DoExpressCheckoutPayment or DoDirectPayment.
            'amt' => '20.00', // Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
            ...
            'invnum' => '12346', // Your invoice number
    ...
    function Do_authorization() {
        $DAFields = array(
            'transactionid' => '123456', // Required.  The value of the order's transaction ID number returned by PayPal.  
            'amt' => '20', // Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
            'transactionentity' => '7', // Type of transaction to authorize.  The only allowable value is Order, which means that the transaction represents a customer order that can be fulfilled over 29 days.
            'currencycode' => 'USD', // Three-character currency code.
            'msgsubid' => '332211'      // A message ID used for idempotence to uniquely identify a message.
        );
    ...
    function Do_reauthorization() {
        $DRFields = array(
            'authorizationid' => '33A85829YU969820D', // Required. The value of a previously authorized transaction ID returned by PayPal.
            'amt' => '20', // Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
            'currencycode' => 'USD', // Three-character currency code.
    ...
    function Do_direct_payment() {
        ...
        $CCDetails = array(
            ...
            'acct' => '4828724366885280 ', // Required.  Credit card number.  No spaces or punctuation.  
            'expdate' => '4/2019 ', // Required.  Credit card expiration date.  Format is MMYYYY
    ...
    function Do_direct_payment_demo() {
        ...
        $CCDetails = array(
            ...
            'acct' => '374638525834094', //$this->input->post('card_no'), 								//374638525834094 Required.  Credit card number.  No spaces or punctuation.  
            'expdate' => '042019', // Required.  Credit card expiration date.  Format is MMYYYY
        ...
        $PayerName = array(
            'salutation' => $this->input->post('title'), // Payer's salutation.  20 char max.
            'firstname' => $this->input->post('frist_name'), // Payer's first name.  25 char max.
            'middlename' => '', // Payer's middle name.  25 char max.
            'lastname' => $this->input->post('last_name'), // Payer's last name.  25 char max.
            'suffix' => ''        // Payer's suffix.  12 char max.
        );

        $BillingAddress = array(
            'street' => '20 south avenue', // Required.  First street address.
            'street2' => '', // Second street address.
            'city' => 'Colorado', // Required.  Name of City.
            'state' => 'FL', // Required. Name of State or Province.
            'countrycode' => 'US', // Required.  Country code.
            'zip' => '30208', // Required.  Postal code of payer.
        ...
        $PaymentDetails = array(
            'amt' => '20.00', // Required.  Total amount of order, including shipping, handling, and tax.  
            ...
            'handlingamt' => '', // Total handling costs for the order.  If you specify handlingamt, you must also specify itemamt.
            'taxamt' => '', // Required if you specify itemized cart tax details. Sum of tax for all items on the order.  Total sales tax. 
            'desc' => 'Purchase ', // Description of the order the customer is purchasing.  127 char max.
            'custom' => '', // Free-form field for your own use.  256 char max.
            'invnum' => '', // Your own invoice or tracking number
            'notifyurl' => ''      // URL for receiving Instant Payment Notifications.  This overrides what your profile is set to use.
        );
        ...
        $Item = array(
            ...
            'l_amt' => '20.00', // Cost of individual item.
    ...
    function Set_express_checkout() {
        $SECFields = array(
            'token' => '2014-05-08T13:17:10Z', // A timestamped token, the value of which was returned by a previous SetExpressCheckout call.
            'maxamt' => '20', // The expected maximum total amount the order will be, including S&H and sales tax.
            'returnurl' => 'http://localhost/studio_1/student/purchase/2/2', // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
            'cancelurl' => base_url(), // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
            'callback' => '', // URL to which the callback request from PayPal is sent.  Must start with https:// for production.
            'callbacktimeout' => base_url(), // An override for you to request more or less time to be able to process the callback request and response.  Acceptable range for override is 1-6 seconds.  If you specify greater than 6 PayPal will use default value of 3 seconds.
            ...
            'addroverride' => '0', // The value 1 indiciates that the PayPal pages should display the shipping address set by you in the SetExpressCheckout request, not the shipping address on file with PayPal.  This does not allow the customer to edit the address here.  Must be 1 or 0.
        ...
        $Payment = array(
            ...
            'itemamt' => '20.00', // Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
        ...
        $Item = array(
            'name' => 'dance', // Item name. 127 char max.
            'desc' => 'course', // Item description. 127 char max.
            'amt' => '20.00', // Cost of item.
            'number' => '1234', // Item number.  127 char max.
    ...
    function Set_auth_flow_param() {
        $SetAuthFlowParamFields = array(
            'ReturnURL' => base_url(), // URL to which the customer's browser is returned after choosing to authenticate with PayPal
            'CancelURL' => base_url(), // URL to which the customer is returned if they decide not to log in.
            'LogoutURL' => base_url(), // URL to which the customer is returned after logging out from your site.
    ...
    function Set_access_permissions() {
        $SetAccessPermissionsFields = array(
            'ReturnURL' => base_url(), // URL to return the browser to after authorizing permissions.
            'CancelURL' => base_url(), // URL to return if the customer cancels authorization
