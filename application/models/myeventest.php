...
class Myeventest extends CI_Controller {

    public function getAllBatch($email) {

        $query = $this->db->query('select tbl_batch.BatchID, tbl_batch.CourseID, tbl_useronbatch.UserID,tbl_batch.StartDate,tbl_batch.EndDate,tbl_course.CourseName from tbl_batch inner join tbl_course on tbl_course.CourseID = tbl_batch.CourseID Inner Join tbl_useronbatch on tbl_useronbatch.BatchID = tbl_batch.BatchID where tbl_useronbatch.UserID = "' . $email . '"');
