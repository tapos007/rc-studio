...
function template_functions() {
    ...
    $(".sortable").sortable({revert: true, cancel: ".btn,.box-content,.nav-header", update: function(b, c) {
        }});
    ...
    $(".datatable").dataTable({sDom: "<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span12'i><'span12 center'p>>", sPaginationType: "bootstrap", oLanguage: {sLengthMenu: "_MENU_ records per page"}});
    ...
    $(".btn-minimize").click(function(c) {
        ...
        if (b.is(":visible")) {
            $("i", $(this)).removeClass("glyphicon glyphicon-chevron-up").addClass("glyphicon glyphicon-chevron-down")
        } else {
            $("i", $(this)).removeClass("glyphicon glyphicon-chevron-down").addClass("glyphicon glyphicon-chevron-up")
        }
        b.slideToggle()
    });
    ...
    $(".progressAnimate").progressbar({value: 1, create: function() {
            $(".progressAnimate .ui-progressbar-value").animate({width: "100%"}, {duration: 10000, step: function(b) {
                    $(".progressAnimateValue").html(parseInt(b) + "%")
                }, easing: "linear"})
        }});
    $(".progressUploadAnimate").progressbar({value: 1, create: function() {
            $(".progressUploadAnimate .ui-progressbar-value").animate({width: "100%"}, {duration: 20000, easing: "linear", step: function(b) {
    ...
    if ($(".progressBarValue")) {
        $(".progressBarValue").each(function() {
            ...
            var b = parseInt(c);
            $(this).find(".progressCustomValue").progressbar({value: 1, create: function() {
                    $(this).find(".ui-progressbar-value").animate({width: b + "%"}, {duration: 5000, step: function(d) {
    ...
    $(".sliderSimple").slider();
    $(".sliderMin").slider({range: "min", value: 180, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMinLabel").html("$" + c.value)
        }});
    $(".sliderMin-1").slider({range: "min", value: 50, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMin1Label").html("$" + c.value)
        }});
    $(".sliderMin-2").slider({range: "min", value: 100, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMin2Label").html("$" + c.value)
        }});
    $(".sliderMin-3").slider({range: "min", value: 150, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMin3Label").html("$" + c.value)
        }});
    $(".sliderMin-4").slider({range: "min", value: 250, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMin4Label").html("$" + c.value)
        }});
    $(".sliderMin-5").slider({range: "min", value: 350, min: 1, max: 700, slide: function(b, c) {
            $(".sliderLabel").html("$" + c.value)
        }});
    $(".sliderMin-6").slider({range: "min", value: 450, min: 1, max: 700, slide: function(b, c) {
            $(".sliderLabel").html("$" + c.value)
        }});
    $(".sliderMin-7").slider({range: "min", value: 550, min: 1, max: 700, slide: function(b, c) {
            $(".sliderLabel").html("$" + c.value)
        }});
    $(".sliderMin-8").slider({range: "min", value: 650, min: 1, max: 700, slide: function(b, c) {
            $(".sliderLabel").html("$" + c.value)
        }});
    $(".sliderMax").slider({range: "max", value: 280, min: 1, max: 700, slide: function(b, c) {
            $(".sliderMaxLabel").html("$" + c.value)
        }});
    $(".sliderRange").slider({range: true, min: 0, max: 500, values: [192, 470], slide: function(b, c) {
...
function circle_progess() {
    var a = $("div");
    if (retina()) {
        $(".whiteCircle").knob({min: 0, max: 100, readOnly: true, width: 240, height: 240, bgColor: "rgba(255,255,255,0.5)", fgColor: "rgba(255,255,255,0.9)", dynamicDraw: true, thickness: 0.2, tickColorizeValues: true});
        $(".circleStat").css("zoom", 0.5);
        $(".whiteCircle").css("zoom", 0.999)
    } else {
        $(".whiteCircle").knob({min: 0, max: 100, readOnly: true, width: 120, height: 120, bgColor: "rgba(255,255,255,0.5)", fgColor: "rgba(255,255,255,0.9)", dynamicDraw: true, thickness: 0.2, tickColorizeValues: true})
    }
    $(".circleStatsItemBox").each(function() {
        ...
        $(this).find(".count > .number").countTo({from: 0, to: endValue, speed: countSpeed, refreshInterval: 50, onComplete: function(e) {
                console.debug(this)
            }})
    });
    $(".circleChart").each(function() {
        var b = $(this).parent().css("color");
        $(this).knob({min: 0, max: 100, readOnly: true, width: 120, height: 120, fgColor: b, dynamicDraw: true, thickness: 0.2, tickColorizeValues: true, skin: "tron"})
    });
    $(".knob").each(function() {
        $(this).knob()
    })
...
function calendars() {
    ...
    $("#main_calendar").fullCalendar({header:
                {
                    left: "title", right: "prev,next today,month,agendaWeek,agendaDay"
                }, editable: true, events:
                [
                    {title: "All Day Event", start: new Date(e, a, 1)},
                    {title: "Long Event", start: new Date(e, a, c - 5), end: new Date(e, a, c - 2)},
                    {id: 999, title: "Repeating Event",
                        //start:new Date(e,a,c-3,16,0),allDay:false},{id:999,title:"Repeating Event",
                        //start:new Date(e,a,c+4,16,0),allDay:false},{title:"Meeting",
                        //start:new Date(e,a,c,10,30),allDay:false},{title:"Lunch",
                        //start:new Date(e,a,c,12,0),end:new Date(e,a,c,14,0),allDay:false},{title:"Birthday Party",
                        //start:new Date(e,a,c+1,19,0),end:new Date(e,a,c+1,22,30),allDay:false
                    }
                ]
    });
    /*var shodate = new Date('February 18, 2014 12:00:00');
     $(".calendar-small")	
     .fullCalendar({
     header:{
     right:"title",
     ...
     start:new Date(shodate.getFullYear(),shodate.getMonth(),shodate.getDate(),12,0),
     end:new Date(shodate.getFullYear(),shodate.getMonth(),shodate.getDate(),14,0),allDay:false
     },
     {title:"Shofi Vhai",start:new Date(b.getFullYear(),b.getMonth(),b.getDate()-1),allDay:true},
     {title:"Guitar Basic",start:new Date(b.getFullYear(),b.getMonth(),b.getDate()-4),allDay:true},
     {title:"Cooking Class",url:"http://google.com/"}
     ],
     
     dayClick:function(g,j,f,d){$(".calendar-details > .events").html("");
    ...
    $("#main_calendar_phone").fullCalendar({
        ...
        defaultView: "agendaDay", editable: true, events: [
            {
                title: "All Day Event", start: new Date(e, a, 1)
            },
            {
...
function growlLikeNotifications() {
    $("#add-sticky").click(function() {
        var a = $.gritter.add({title: "This is a sticky notice!", text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus eget tincidunt velit. Cum sociis natoque penatibus et <a href="#" style="color:#ccc">magnis dis parturient</a> montes, nascetur ridiculus mus.', image: "img/avatar.jpg", sticky: true, time: "", class_name: "my-sticky-class"});
        return false
    });
    $("#add-regular").click(function() {
        $.gritter.add({title: "This is a regular notice!", text: 'This will fade out after a certain amount of time. Vivamus eget tincidunt velit. Cum sociis natoque penatibus et <a href="#" style="color:#ccc">magnis dis parturient</a> montes, nascetur ridiculus mus.', image: "img/avatar.jpg", sticky: false, time: ""});
        return false
    });
    $("#add-max").click(function() {
        $.gritter.add({title: "This is a notice with a max of 3 on screen at one time!", text: 'This will fade out after a certain amount of time. Vivamus eget tincidunt velit. Cum sociis natoque penatibus et <a href="#" style="color:#ccc">magnis dis parturient</a> montes, nascetur ridiculus mus.', image: "img/avatar.jpg", sticky: false, before_open: function() {
    ...
    $("#add-with-callbacks").click(function() {
        $.gritter.add({title: "This is a notice with callbacks!", text: "The callback is...", before_open: function() {
    ...
    $("#add-sticky-with-callbacks").click(function() {
        $.gritter.add({title: "This is a sticky notice with callbacks!", text: "Sticky sticky notice.. sticky sticky notice...", sticky: true, before_open: function() {
    ...
    $("#remove-all-with-callbacks").click(function() {
        $.gritter.removeAll({before_close: function(a) {
...
$.fn.dataTableExt.oApi.fnPagingInfo = function(a) {
    return{iStart: a._iDisplayStart, iEnd: a.fnDisplayEnd(), iLength: a._iDisplayLength, iTotal: a.fnRecordsTotal(), iFilteredTotal: a.fnRecordsDisplay(), iPage: Math.ceil(a._iDisplayStart / a._iDisplayLength), iTotalPages: Math.ceil(a.fnRecordsDisplay() / a._iDisplayLength)}
};
